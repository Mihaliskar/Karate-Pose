...
Writes:
    ...
    splits/validation_indices.npy
    splits/test_indices.npy
    splits/split_assignments.csv

Samples with the same (person_id, group_id) are always kept
in the same split.

Target labels use only class_name.
The original left/right side is retained only for analysis.
"""
...
def load_feature_metadata(
    ...
    if not feature_path.is_file():
        raise FileNotFoundError(
            f"Feature archive not found: {feature_path}"
        )
    ...
    with np.load(feature_path, allow_pickle=False) as archive:
        ...
        if missing_arrays:
            raise ValueError(
                f"Feature archive is missing arrays: "
        ...
        paths = archive["paths"].astype(
            str,
            copy=True,
        )

        class_names = archive["class_names"].astype(
            str,
            copy=True,
        )

        sides = archive["sides"].astype(
            str,
            copy=True,
        )

        person_ids = archive["person_ids"].astype(
            str,
            copy=True,
        )

        group_ids = archive["groups"].astype(
            str,
            copy=True,
        )
    ...
    for name, array in arrays.items():
        if len(array) != sample_count:
            raise ValueError(
                f"Array {name!r} has {len(array)} rows; "
                f"expected {sample_count}"
            )

    if sample_count == 0:
        raise ValueError(
            "Feature archive contains no samples"
        )

    if len(class_names) == 0:
        raise ValueError(
            "Feature archive contains no class names"
        )

    if np.any(y < 0) or np.any(y >= len(class_names)):
        raise ValueError(
            "The y array contains labels outside class_names"
        )

    if np.any(np.char.strip(person_ids) == ""):
        raise ValueError(
            "person_ids contains empty values"
        )

    if np.any(np.char.strip(group_ids) == ""):
        raise ValueError(
            "groups contains empty values"
        )
...
def make_recording_groups(
    ...
    """Make recording IDs unique across people."""

    return np.asarray(
        [
            f"person={person}|group={group}"
...
def create_split_indices(
    ...
    # Verify that each recording group belongs to exactly one class.
    ...
    if mixed_groups:
        raise ValueError(
            "Some recording groups contain multiple classes. "
            "The current class-by-class allocator cannot safely "
            "split these groups. Examples: "
            f"{mixed_groups[:5]}"
        )

    for class_index in range(len(np.unique(y))):
        class_sample_indices = np.flatnonzero(
            y == class_index
        )

        class_groups = np.unique(
            recording_groups[class_sample_indices]
        )
        ...
        # Dynamic programming over achievable validation/test
        # sample counts.
        #
        # Assignment:
        #   0 = train
        ...
        def state_score(
            ...
            validation_test_difference = abs(
                validation_count
                - test_count
            )
        ...
        for group, assignment in zip(
            ...
            split_name = (
                split_name_for_assignment[assignment]
            )

            split_indices[split_name].extend(
                indices_by_group[group].tolist()
            )
...
def validate_splits(
    ...
    if len(all_indices) != len(y):
        raise RuntimeError(
            f"Splits contain {len(all_indices)} indices "
            f"for {len(y)} samples"
        )
    ...
    if len(unique_indices) != len(y):
        raise RuntimeError(
            "Some samples appear in multiple splits"
        )
    ...
    if not np.array_equal(
        ...
        raise RuntimeError(
            "Some dataset samples are missing from the splits"
        )

    split_group_sets = {
        name: set(
            recording_groups[indices].tolist()
        )
    ...
    for first_position, first_name in enumerate(
        split_names
    ):
        for second_name in split_names[
            ...
            overlap = (
                split_group_sets[first_name]
                & split_group_sets[second_name]
            )

            if overlap:
                raise RuntimeError(
                    "Recording-group leakage between "
...
def print_summary(
    ...
    for split_name, indices in splits.items():
        ...
        print(
            ...
            f"({percentage:.1f}%), "
            f"{group_count} recording groups"
        )
...
def main() -> None:
    ...
    recording_groups = make_recording_groups(
        person_ids,
        group_ids,
    )
    ...
    print(
        ...
        f"{len(np.unique(recording_groups))}"
    )
